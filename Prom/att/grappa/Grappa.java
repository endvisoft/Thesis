 ...
 * convenience methods for exception display are included and some lookup tables
 * are initialized.
 * 
 * <P>
 ...
 * <LI>drawing a positioned graph.
 * </UL>
 * Grappa itself does not have any methods for graph positioning. Grappa simply
 * draws the nodes and edges based on the value of the element's <TT>pos</TT>
 * attribute, though it will treat an unpositioned edge as a straight line
 * between the center points of its attached nodes.
 * 
 * Some graph layout references are:
 * <OL>
 * <LI>"An Algorithm for Drawing General Undirected Graphs", Tomihisa Kamada and
 * Satoru Kawai, <I>Information Processing Letters</I>, Vol. 31 (1989), pp. 7 -
 * 15
 * 
 * <LI>"Graph Drawing by Force-directed Placement", Thomas M. J. Fruchterman and
 * Edward M. Reingold, <I>Software - Practice and Experience</I>, Vol. 21 No. 11
 * (1991), pp. 1129 - 1164
 * 
 * <LI>"A Technique for Drawing Directed Graphs", Emden R. Gansner, Eleftherios
 * Koutsofios, Stephen C. North and Kiem-Phong Vo, <I>IEEE Transactions on
 * Software Engineering</I>, Vol 19 No. 3 (1993), pp. 214 - 230
 * 
 * <LI>"NicheWorks - Interactive Visualization of Very Large Graphs", Graham J.
 * Wills, <A
 * HREF="http://www.bell-labs.com:80/user/gwills/NICHEguide/nichepaper.html"
 * TARGET
 * ="_blank">http://www.bell-labs.com:80/user/gwills/NICHEguide/nichepaper.
 * html</A>, circa 1997
 * </OL>
 * 
 * <P>
 * Grappa does supply a utility, GrappaSupport.filterGraph(), for updating a
 * graph (including position information) from I/O streams such as might be
 * obtained from a java.net.URLConnection.
 * 
 ...
 */
public abstract class Grappa implements att.grappa.GrappaConstants {
	...
	/*
	 * Set-up lookup table that defines recognized shapes (useful for switch
	 * statements).
	 */
	static {
		...
		java.util.Enumeration enu = keyToShape.keys();
		String key = null;
	...
	/*
	 * A instance of ExceptionDisplay used with the (public) convenience methods
	 * that follow.
	 */
	private static final ExceptionDisplay exceptionDisplay = new ExceptionDisplay(
			Grappa.PKG_UPLOW + ":  Exception Detected");
	 ...
	 * be <I>Grappa.emptyEnumeration.elements()</I>, whose
	 * <I>hasMoreElements()</I> method will return <B>false</B>).
	 */
	public static final java.util.Vector emptyEnumeration = new java.util.Vector(
			0, 0);

	/*
	 * Default tool-tip text when cursor is outside graph, but inside the
	 * display panel.
	 */
	private static String toolTipText = "<outside graph boundary>";

	/**
	 * Sets the tool-tip text displayed when outside graph, but inside the
	 * display panel.
	 * 
	 * @param text
	...
	/**
	 * Boolean to indicate if all element attributes should be printed. By
	 * default, only the element specific attributes are printed and attributes
	 * inherited from a parent element (subgraph) are skipped.
	 * 
	...
	/**
	 * Boolean to indicate if the default attributes associated with a subgraph
	 * should be printed. By default, only those differing from the pre-defined
	 ...
	 * Otherwise, the position is assumed to indicate the upper-left corner of
	 * the nodes bounding box. By default, the position indicates the node's
	 * center point.
	 */
	public static boolean centerPointNodes = true;
	/**
	 * Indicates if the label position for node labels should automatically be
	 * set to the center point of the node. When true, positioning information
	 * provided by the <TT>lp</TT> attribute is completely ignored. By default,
	 * auto-positioning is used.
	 */
	...
	/**
	 * Indicates if the <TT>bb</TT> attribute of a subgraph should automatically
	 * be set whenever the bounding box is calculated. By default, the attribute
	 ...
	 * The default is the java.awt.geom.PathIterator WIND_NON_ZERO rule.
	 */
	public static int windingRule = java.awt.geom.PathIterator.WIND_NON_ZERO;

	/**
	 * Indicates whether the <TT>orientation</TT> attribute is specifed in
	 * degrees rather than radians. Degrees is the default.
	 */
	public static boolean orientationInDegrees = true;
	/**
	 * Indicates whether the <TT>rotation</TT> attribute is specifed in degrees
	 * rather than radians. Degrees is the default.
	 */
	public static boolean rotationInDegrees = true;

	/**
	 * Indicates whether only the list of attributes found in the PRINTLIST_ATTR
	 * should be printed. See also printVisibleOnly. The default is false.
	 */
	public static boolean usePrintList = false;

	/**
	 * Indicates whether only visible elements should be included when printing
	 * a graph. The default is false.
	 */
	public static boolean printVisibleOnly = false;

	/**
	 * Indicates whether anti-aliasing should be used when drawing. The default
	 * is true.
	 */
	public static boolean useAntiAliasing = true;

	/**
	 * Indicates whether anti-aliasing should be used when drawing text. The
	 * default is false.
	 */
	...
	/**
	 * Indicates whether the value of y-coordinates should be negated when
	 * reading or writing y-coord information as string attributes. Note: this
	 * indicator should be set to <I>true</I> when working with string
	 * attributes generated by or to be read by the <TT>dot</TT> graph layout
	 * program or to be compatible to earlier versions of Grappa. The default is
	 * true.
	 */
	public static boolean negateStringYCoord = true;

	/**
	 * Indicates that graph labels, when not explicitly positioned via the
	 * <TT>lp</TT> attribute, should be placed at the bottom of the graph
	 * instead of the top. The default is true, meaning the label will be placed
	 * at the bottom of the graph.
	 */
	public static boolean labelGraphBottom = true;
	/**
	 * Indicates that graph labels, when not explicitly positioned via the
	 * <TT>lp</TT> attribute, should be placed just outside the graph bounding
	 * box instead of just inside. The default is true, meaning the label will
	 * be placed outside the bounding box.
	 */
	public static boolean labelGraphOutside = true;

	/**
	 * Indicates that background drawing, if any is provided via a
	 * <TT>GrappaBacker</TT> implementation, should be displayed or not. The
	 * default of true means the background drawing should be displayed, if
	 * provided.
	 */
	public static boolean backgroundDrawing = true;

	/**
	 * When the transform scale applied when drawing in a GrappaPanel is less
	 * than this value, then node labels are suppressed.
	 * 
	 */
	public static double nodeLabelsScaleCutoff = 0.5;

	/**
	 * Cluster subgraphs will have their bounding box outlined. To similarly
	 * outline all types of subgraphs (except the root subgraph), set this value
	 * to true.
	 * 
	 */
	...
	/**
	 * When the transform scale applied when drawing in a GrappaPanel is less
	 * than this value, then edge labels are suppressed.
	 * 
	 */
	...
	/**
	 * When the transform scale applied when drawing in a GrappaPanel is less
	 * than this value, then subgraph labels are suppressed.
	 * 
	 */
	...
	/**
	 * Indicates that an image requested via the IMAGE_ATTR of an element should
	 * be loaded before the element is drawn. By default, Grappa will wait.
	 * 
	...
	/**
	 * Indicates which classes of elements are suitable for selection based on
	 * cursor position. Value is the logical OR of NODE, EDGE and SUBGRAPH. The
	 * default is SUBGRAPH|NODE|EDGE.
	 * 
	 */
