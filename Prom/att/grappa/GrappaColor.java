...
public abstract class GrappaColor {
	...
	public static void addColor(String name, Color color)
		...
		if (canonName == null) {
			throw new IllegalArgumentException(
					"supplied name does not contain alphabetics (" + name + ")");
		}
	...
	private static String canonColor(String name, float[] hsb) {
		...
		if (hsb != null && allDigits && commas == 2 && dots <= 3) {
			...
			try {
				for (int i = 0; i < 3; i++) {
					hsb[i] = Float.valueOf(name.substring(prev, commaSpots[i]))
							.floatValue();
					prev = commaSpots[i] + 1;
				}
	...
	/**
	 * Return the color in the color table with the given name. If the color is
	 * not found, the supplied default is returned. If the supplied default is
	 * null, the class default is returned. If the name consists of three comma
	 * or space separated floating point numbers in the range 0 to 1 inclusive,
	 * then it is assumed to represent an HSB color specification and generated
