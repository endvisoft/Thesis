...
/**
 * This class provides a flexible, parameterized polygonal shape builder. The
 * guts of a GrappaShape is a GeneralPath object.
 * 
...
public class GrappaShape implements GrappaConstants, Cloneable, Shape {
	...
	protected final static double CIRCLE_XDIAG = Math.sqrt(7.0 / 16.0);
	protected final static double CIRCLE_YDIAG = 0.75;
	...
	/**
	 * Constructs a new <code>GrappaShape</code> object. The winding rule for
	 * this path is defaulted (from Grappa.windingRule).
	 * 
	 * @param type
	 *            the shape specifier (.e.g., EGG_SHAPE).
	 * @param x
	 *            the x-coordinate of the polygon center point.
	 * @param y
	 *            the y-coordinate of the polygon center point.
	 * @param width
	 *            the overall width of the polygon bounding box.
	 * @param height
	 *            the overall height of the polygon bounding box.
	 * @param sidesArg
	 *            the number of sides of the polygon.
	 * @param peripheriesArg
	 *            the number of peripheries (outlines) of the polygon.
	 * @param distortionArg
	 *            a distortion factor applied to the polygon shape, when
	 *            non-zero
	 * @param skewArg
	 *            a skewing factor applied to the polygon shape, when non-zero
	 * @param orientationArg
	 *            an orientation angle in radians.
	 * @param roundedArg
	 *            a directive to round the corners.
	 * @param diagonalsArg
	 *            a directive to draw catercorners at the vertices.
	 * @param extra
	 *            used only with the <I>record</I> shape at present, it provides
	 *            a string of tuples giving interior partition information.
	 */
	public GrappaShape(int type, double x, double y, double width,
		...
		for (int j = 0; j < peripheries; j++) {
			...
			if ((rounded || diagonals) && type != OVAL_SHAPE
					&& j == (peripheries - 1)) {
				tooSmall = false;
				tverts = 0;
				Pt0 = rawVertices[0];
				for (i = 0; i < sides; i++) {
					...
					if (!diagonals) {
						tmp2 = RBCURVE * tmp1;
						tmpVertices[tverts++] = new GrappaPoint(Pt0.x + tmp2
								* (Pt1.x - Pt0.x), Pt0.y + tmp2
								* (Pt1.y - Pt0.y));
					}
					tmpVertices[tverts++] = new GrappaPoint(Pt0.x + tmp1
							* (Pt1.x - Pt0.x), Pt0.y + tmp1 * (Pt1.y - Pt0.y));
					tmp1 = 1 - tmp1;
					tmpVertices[tverts++] = new GrappaPoint(Pt0.x + tmp1
							* (Pt1.x - Pt0.x), Pt0.y + tmp1 * (Pt1.y - Pt0.y));
					if (!diagonals) {
				...
				if (tooSmall) {
					for (i = 0; i < sides; i++) {
						if (i == 0) {
							path.moveTo((float) (x + rawVertices[i].x),
									(float) (y - rawVertices[i].y));
						} else {
							path.lineTo((float) (x + rawVertices[i].x),
									(float) (y - rawVertices[i].y));
						}
					}
				} else {
					if (diagonals) {
						path.moveTo((float) (x + tmpVertices[0].x),
								(float) (y - tmpVertices[0].y));
						for (i = (2 * sides) - 1; i > 0; i -= 2) {
							path.lineTo((float) (x + tmpVertices[i].x),
									(float) (y - tmpVertices[i].y));
							path.moveTo((float) (x + tmpVertices[i - 1].x),
									(float) (y - tmpVertices[i - 1].y));
						}
						for (i = 0; i < sides; i++) {
							if (i == 0) {
								path.moveTo((float) (x + rawVertices[i].x),
						...
						path.moveTo((float) (x + tmpVertices[2].x),
								(float) (y - tmpVertices[2].y));
						for (i = 3; i < (4 * sides) - 2; i += 4) {
							path.curveTo((float) (x + tmpVertices[i].x),
									(float) (y - tmpVertices[i].y),
									(float) (x + tmpVertices[i + 1].x),
									(float) (y - tmpVertices[i + 1].y),
									(float) (x + tmpVertices[i + 2].x),
									(float) (y - tmpVertices[i + 2].y));
							path.lineTo((float) (x + tmpVertices[i + 3].x),
									(float) (y - tmpVertices[i + 3].y));
						}
						i = (4 * sides) - 1;
						path.curveTo((float) (x + tmpVertices[i].x),
								(float) (y - tmpVertices[i].y),
								(float) (x + tmpVertices[0].x),
								(float) (y - tmpVertices[0].y),
								(float) (x + tmpVertices[1].x),
								(float) (y - tmpVertices[1].y));
					}
				}
			} else {
				for (i = 0; i < sides; i++) {
		...
		if (type == OVAL_SHAPE && diagonals) {
			...
			Pt0 = new GrappaPoint(width * CIRCLE_XDIAG / 2.0, height
					* CIRCLE_YDIAG / 2.0);
			Ppt.x = x + Pt0.x;
			Ppt.y = y - Pt0.y;
			...
			if (rects != null) {
				...
				for (i = 0; i < rects.length; i += 4) {
					...
					if (!(rects[i] == rectsMin.x && rects[i + 2] == rectsMax.x)
							|| rects[i + 1] != rectsMin.y) {
						path.moveTo(rects[i], rects[i + 1]);
						path.lineTo(rects[i + 2], rects[i + 1]);
					}
					if (rects[i + 2] != rectsMax.x
							|| !(rects[i + 1] == rectsMin.y && rects[i + 3] == rectsMax.y)) {
						path.moveTo(rects[i + 2], rects[i + 1]);
						path.lineTo(rects[i + 2], rects[i + 3]);
					}
					if (!(rects[i] == rectsMin.x && rects[i + 2] == rectsMax.x)
							|| rects[i + 3] != rectsMax.y) {
