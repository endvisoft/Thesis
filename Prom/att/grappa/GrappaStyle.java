...
/**
 * This class translates and encapsulates information provided by the
 * <I>style</I> attribute.
 * 
...
public class GrappaStyle implements GrappaConstants, Cloneable {
	...
	/**
	 * A style attribute with this string value gets set to the default style
	 * for the associated element type.
	 */
	 ...
	 * Line color default.
	 */
	public final static Color STYLE_LINE_COLOR_DEFAULT = GrappaColor.getColor(
			"black", null);
	/**
	 * Line style default.
	 */
	...
	static BasicStroke defaultStroke = new BasicStroke(
			STYLE_LINE_WIDTH_DEFAULT, STYLE_CAP_DEFAULT, STYLE_JOIN_DEFAULT,
			STYLE_MITER_LIMIT_DEFAULT, STYLE_DASH_DEFAULT,
			STYLE_DASH_PHASE_DEFAULT);

	static String defaultStrokeString = generateStrokeString(
			STYLE_LINE_WIDTH_DEFAULT, STYLE_CAP_DEFAULT, STYLE_JOIN_DEFAULT,
			STYLE_MITER_LIMIT_DEFAULT, STYLE_DASH_DEFAULT,
			STYLE_DASH_PHASE_DEFAULT);
	...
	/**
	 * Constructs a new <code>GrappaStyle</code> object from a style description
	 * string.
	 * 
	 * @param type
	 *            element type to associate with this style.
	 * @param style
	 *            the <code>String</code> that specifies the style info. format
	 *            is: style1,style2(extra2),...,styleN.
	 */
	public GrappaStyle(int type, String style) {
		if ((type & (Grappa.NODE | Grappa.EDGE | Grappa.SUBGRAPH | Grappa.SYSTEM)) != type) {
			throw new RuntimeException(
					"type must specify node, edge or subgraph");
		}
	...
	public void updateStyle(String style) {
		...
		if (len == 0 || style.equals(GrappaStyle.DEFAULT_SET_STRING)) {
			return;
		}
		...
		while (i < len) {
			...
			if (c == '(') {
				if (in_parens) {
					throw new RuntimeException(
							"style attribute has nested parentheses");
				}
				if (!parens_ok) {
					throw new RuntimeException(
							"style attribute has parentheses without keyword");
				}
				if (last_option < 0) {
					throw new RuntimeException("style attribute ("
							+ option_string + ") has unexpected modifier");
				}
				in_parens = true;
				keyword_ok = true;
				parens_ok = false;
				...
				if (!in_parens) {
					throw new RuntimeException(
							"style attribute has unmatched right parenthesis");
				}
				in_parens = false;
				keyword_ok = false;
				parens_ok = false;
				...
				if (in_parens) {
					throw new RuntimeException(
							"style attribute has comma within parentheses");
				}
				keyword_ok = true;
				parens_ok = false;
			} else {
				if (!keyword_ok) {
					throw new RuntimeException("style attribute ("
							+ option_string + ") has bad format");
				}
				offset = i - 1;
				if (in_parens) {
					int pcnt = 0;
					while (i < len
							&& ((c = style.charAt(i)) != ')' || pcnt > 0)) {
						if (c == '(') {
							pcnt++;
						} else if (c == ')') {
							pcnt--;
					...
					while (i < len && (c = style.charAt(i)) != ',' && c != '('
							&& c != ')') {
						i++;
					}
				}
				...
				if (in_parens) {
					if (last_option == -1) {
						throw new RuntimeException("style attribute modifier ("
					...
					case STYLE_DASH:
						...
						try {
							dash = GrappaSupport
									.floatArrayForTuple(option_string);
						} catch (NumberFormatException nfe) {
							throw new RuntimeException(
									"dash style attribute modifier ("
											+ option_string
											+ ") is not a comma-delimited floating point tuple");
						}
						break;
					case STYLE_DASH_PHASE:
						try {
							dash_phase = Float.valueOf(option_string)
									.floatValue();
						} catch (NumberFormatException nfe) {
							throw new RuntimeException(
									"dash phase style attribute modifier ("
											+ option_string
					...
					case STYLE_LINE_WIDTH:
						try {
							line_width = Float.valueOf(option_string)
									.floatValue();
						} catch (NumberFormatException nfe) {
							throw new RuntimeException(
									"line width style attribute modifier ("
											+ option_string
											+ ") is not a floating point number");
						}
						break;
					case STYLE_MITER_LIMIT:
						try {
							miter_limit = Float.valueOf(option_string)
									.floatValue();
						} catch (NumberFormatException nfe) {
							throw new RuntimeException(
									"miter limit style attribute modifier ("
											+ option_string
											+ ") is not a floating point number");
						}
						break;
					...
					case STYLE_DIAGONALS:
						diagonals = Boolean.valueOf(option_string)
								.booleanValue();
						break;
					case STYLE_ROUNDED:
					...
					case STYLE_FIXED_SIZE:
						fixed_size = Boolean.valueOf(option_string)
								.booleanValue();
						break;
					case STYLE_OLD_BOLD:
					...
					if (option_obj == null || !(option_obj instanceof Integer)) {
						throw new RuntimeException("style attribute ("
								+ option_string
								+ ") is unrecognized or badly implemented");
					}
		...
		if (in_parens) {
			throw new RuntimeException(
					"style attribute has unmatched left parenthesis");
		}

		String strokeString = generateStrokeString(line_width, cap, join,
				miter_limit, dash, dash_phase);
		if ((stroke = (BasicStroke) strokeCache.get(strokeString)) == null) {
			stroke = new BasicStroke(line_width, cap, join, miter_limit, dash,
					dash_phase);
			strokeCache.put(strokeString, stroke);
		}
	}

	private static String generateStrokeString(float lineWidth, int capType,
			int joinType, float miterLimit, float[] dashSpec, float dashPhase) {
	...
	public String toAttributeString() {
		return (generateStyleString(line_color, line_style, line_width, cap,
				join, miter_limit, dash, dash_phase, rounded, diagonals,
				filled, invis, fixed_size, font_style, false, elementType));
	}
	...
	public String toString() {
		return (generateStyleString(line_color, line_style, line_width, cap,
				join, miter_limit, dash, dash_phase, rounded, diagonals,
				filled, invis, fixed_size, font_style, true, elementType));
	}

	private static String generateStyleString(Color color, int lineStyle,
			float lineWidth, int capType, int joinType, float miterLimit,
			float[] dashSpec, float dashPhase, boolean roundedFlag,
			boolean diagonalsFlag, boolean filledFlag, boolean invisFlag,
			boolean fixedSizeFlag, Integer fontStyle, boolean showAll, int type) {
		...
		if (showAll
			...
			if ((tmpstr = GrappaColor.getColorName(color)) == null) {
				...
				styleStringBuffer.append(GrappaSupportPrintf
						.sprintf(new Object[] { "%g,%g,%g", new Float(hsb[0]),
		...
		if (showAll || lineStyle != STYLE_LINE_STYLE_DEFAULT) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
		...
		if (showAll || lineWidth != STYLE_LINE_WIDTH_DEFAULT) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
		...
		if (showAll || capType != STYLE_CAP_DEFAULT) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
		...
		if (showAll || joinType != STYLE_JOIN_DEFAULT) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
		...
		if (showAll || miterLimit != STYLE_MITER_LIMIT_DEFAULT) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
		...
		if (showAll || dashPhase != STYLE_DASH_PHASE_DEFAULT) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
		...
		if ((type > 0 && (type & Grappa.NODE) == Grappa.NODE)
				&& (showAll || roundedFlag != STYLE_ROUNDED_DEFAULT)) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
		...
		if ((type > 0 && (type & Grappa.NODE) == Grappa.NODE)
				&& (showAll || diagonalsFlag != STYLE_DIAGONALS_DEFAULT)) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
		...
		if ((type > 0 && (type & (Grappa.NODE | Grappa.SUBGRAPH)) != 0)
				&& (showAll || filledFlag != STYLE_FILLED_DEFAULT)) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
		...
		if ((type > 0 && (type & (Grappa.NODE | Grappa.EDGE | Grappa.SUBGRAPH)) != 0)
				&& (showAll || invisFlag != STYLE_INVIS_DEFAULT)) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
		...
		if (showAll || fixedSizeFlag != STYLE_FIXED_SIZE_DEFAULT) {
			if (styleStringBuffer == null) {
				styleStringBuffer = new StringBuffer();
			} else {
				styleStringBuffer.append(',');
	 ...
	 * 
	 * @return the rounded corner specification color (true indicates rounded
	 *         corners).
	 */
	 ...
	 * 
	 * @return the diagonal corner specification color (true indicates diagonal
	 *         corners).
	 */
	 ...
	 * 
	 * @return the invisibility specification (true indicates element is
	 *         invisible).
	 */
	 ...
	 * 
	 * @return the fixed size specification (true indicates that fixed size
	...
	/**
	 * Get the font style. Actually, the <I>fontstyle</I> attribute should be
	 * used for font style dealings rather than the <I>style</I> attribute, but
