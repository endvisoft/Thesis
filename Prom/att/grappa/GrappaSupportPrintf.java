...

{
	...
	/**
	 * The familiar C-language sprintf rendered in Java and extended for some
	 * Grappa types.
	 * 
	 * @param args
	 *            the first element of this array is a string giving the format
	 *            of the returned string, the remaining elements are object to
				...
				if ((n = cvt.parse(format, n)) < flen) {
					...
					case 'b':
						...
						if (args[argn] instanceof java.awt.geom.Rectangle2D) {
							cvt.buildBox(prtbuf,
									((java.awt.geom.Rectangle2D) args[argn++]),
									false, flag);
						} else {
							throw new RuntimeException("argument " + argn
									+ " should be a Rectangle2D");
						}
						break;
					...
					case 'c':
						...
						if (args[argn] instanceof Character) {
							cvt.buildChar(prtbuf, ((Character) args[argn++])
									.charValue());
						} else {
							throw new RuntimeException("argument " + argn
					...
					case 'd':
						...
						if (args[argn] instanceof Number) {
							cvt.buildInteger(prtbuf, ((Number) args[argn++])
									.intValue());
						} else {
							throw new RuntimeException("argument " + argn
									+ " should be a Number");
					...
					case 'o':
						...
						if (args[argn] instanceof Character) {
							cvt.buildOctal(prtbuf, ((Character) args[argn++])
									.charValue());
						} else if (args[argn] instanceof Number) {
							cvt.buildOctal(prtbuf, ((Number) args[argn++])
									.intValue());
						} else {
							throw new RuntimeException("argument " + argn
									+ " should be a Character or Number");
						}
						break;
					...
					case 'p':
						...
						if (args[argn] instanceof java.awt.geom.Point2D) {
							cvt.buildPoint(prtbuf,
									((java.awt.geom.Point2D) args[argn++]),
									false);
						} else if (args[argn] instanceof java.awt.geom.Dimension2D) {
							cvt.buildSize(prtbuf,
									((java.awt.geom.Dimension2D) args[argn++]),
									false);
						} else {
							throw new RuntimeException("argument " + argn
									+ " should be a Point2D");
					...
					case 'x':
						...
						if (args[argn] instanceof Character) {
							cvt.buildHex(prtbuf, ((Character) args[argn++])
									.charValue(), false);
						} else if (args[argn] instanceof Number) {
							cvt.buildHex(prtbuf, ((Number) args[argn++])
									.intValue(), false);
						} else {
							throw new RuntimeException("argument " + argn
									+ " should be a Character or Number");
					...
					case 'e':
						...
						if (args[argn] instanceof Character) {
							cvt.buildExp(prtbuf, ((Character) args[argn++])
									.charValue(), false);
						} else if (args[argn] instanceof Number) {
							cvt.buildExp(prtbuf, ((Number) args[argn++])
									.doubleValue(), false);
						} else {
							throw new RuntimeException("argument " + argn
									+ " should be a Character or Number");
					...
					case 'f':
						...
						if (args[argn] instanceof Character) {
							cvt.buildFloat(prtbuf, ((Character) args[argn++])
									.charValue());
						} else if (args[argn] instanceof Number) {
							cvt.buildFloat(prtbuf, ((Number) args[argn++])
									.doubleValue());
						} else {
							throw new RuntimeException("argument " + argn
									+ " should be a Character or Number");
					...
					case 'g':
						...
						if (args[argn] instanceof Character) {
							cvt.buildFlex(prtbuf, ((Character) args[argn++])
									.charValue(), false);
						} else if (args[argn] instanceof Number) {
							cvt.buildFlex(prtbuf, ((Number) args[argn++])
									.doubleValue(), false);
						} else {
							throw new RuntimeException("argument " + argn
									+ " should be a Character or Number");
				...
				if (n < flen) {
					...
					case '9':
						...
						if ((n + 1) < flen) {
							if (GrappaSupport.isdigit(format[n])
									&& GrappaSupport.isdigit(format[n + 1])) {
								int oct = (int) format[n + 1] + 8
										* (int) format[n] + 64 * (int) ch;
...

{
	...
	final int parse(char cfmt[], int n) {
		...
		if (n < cfmt.length && cfmt[n] == '.') {
			n++;
			for (precision = 0; n < cfmt.length
					&& GrappaSupport.isdigit(ch = cfmt[n]); n++) {
	...
	final StringBuffer buildChar(StringBuffer buf, int arg) {
	...
	final StringBuffer buildExp(StringBuffer buf, double arg, boolean upper) {
		...
		return (strpad(buf, doubleToString(sign * base, scratch.toString()),
				padding, width, rightpad));
	}

	// /////////////////////////////////////////////////////////////////////////

	final StringBuffer buildFlex(StringBuffer buf, double arg, boolean upper) {
	...
	final StringBuffer buildPoint(StringBuffer buf, java.awt.geom.Point2D parg,
			boolean upper) {
	...
	final StringBuffer buildSize(StringBuffer buf,
			java.awt.geom.Dimension2D parg, boolean upper) {
	...
	final StringBuffer buildBox(StringBuffer buf,
			java.awt.geom.Rectangle2D parg, boolean upper, boolean dimensioned) {
	...
	final StringBuffer buildFloat(StringBuffer buf, double arg) {
	...
	final StringBuffer buildHex(StringBuffer buf, int arg, boolean upper) {

		String str;

		scratch.setLength(0);

		str = (upper) ? Integer.toHexString(arg).toUpperCase() : Integer
				.toHexString(arg);
		...
		if (precision > str.length()) {
			...
			if (zeropad && !rightpad && precision < 0) {
				if (alternate) {
					...
					strpad(buf, str, '0', width, rightpad);
				}
			} else {
				if (alternate) {
					scratch.append(upper ? "0X" : "0x");
	...
	final StringBuffer buildInteger(StringBuffer buf, long arg) {
	...
	final StringBuffer buildOctal(StringBuffer buf, int arg) {
	...
	final StringBuffer buildString(StringBuffer buf, String arg) {
	...
	private StringBuffer strpad(StringBuffer buf, String str, int ch,
			int width, boolean right) {
