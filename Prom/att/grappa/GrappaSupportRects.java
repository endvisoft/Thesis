...
/**
 * This class provides a method for parsing RECORD_SHAPE node labels and
 * deriving the RECT_ATTR information from it. It is called by the GrappaNexus
 * class.
 * 
...
public class GrappaSupportRects implements GrappaConstants {
	...
	protected static synchronized Object[] parseRecordInfo(Node node) {
		...
		double height = ((Double) node.getAttributeValue(HEIGHT_ATTR))
				.doubleValue()
				* PointsPerInch;
		Dimension sz = new Dimension((int) Math.round(width), (int) Math
				.round(height));
		tableField.resizeFields(sz);

		GrappaPoint pos = (GrappaPoint) node.getAttributeValue(POS_ATTR);
		tableField.positionFields(new Point((int) Math.round(pos.getX() - width
				/ 2.0), (int) Math.round(pos.getY() - height / 2.0)));
	...
	private static boolean emitFields(TableField tf, Object[] objs) {
		...
		if (fc == 0) {
			...
			rbuf.append(',');
			rbuf.append(Grappa.negateStringYCoord ? (-rect.y - rect.height)
					: (rect.y + rect.height));
			;

			((String[]) objs[0])[fields] = tf.getText();
			((GrappaPoint[]) objs[1])[fields] = new GrappaPoint(rect
					.getCenterX(), rect.getCenterY());
			fields++;
			return true;
		}
	...
	private static TableField doParse(Node node, boolean LR, boolean topLevel) {
		...
		for (int pos = arrayOffset; pos < parseArray.length; pos++) {
			if (parseArray[pos] == '\\') {
				...
				if (pos < parseArray.length
						&& (parseArray[pos] == '{' || parseArray[pos] == '}' || parseArray[pos] == '|')) {
					continue;
				}
			}
			if (parseArray[pos] == '{') {
		...
		while (wflag) {
			...
			case '|':
			case '\000':
				if ((arrayOffset >= parseArray.length && !topLevel)
						|| (mode & INPORT) != 0) {
					return null;
				}
				if ((mode & HASTABLE) == 0) {
...
class TableField implements GrappaConstants {
	...
	void setId(String id) {
		...
		if (hadNBSP) {
			idTag = new String(array, 0, array.length);
		} else {
			idTag = id;
		}
	}
		...
		if (fc == 0) {
			...
			for (int cnt = 0; cnt < fc; cnt++) {
				...
				if (tf.isLR()) {
					...
					dim.height = (dim.height > dtmp.height) ? dim.height
							: dtmp.height;
				} else {
					dim.width = (dim.width > dtmp.width) ? dim.width
							: dtmp.width;
	...
	void resizeUpFields(TableField tf, Dimension sz) {
		...
		Dimension delta = new Dimension(sz.width - tf.getSize().width,
				sz.height - tf.getSize().height);
		tf.setSize(sz);
		...
		for (int cnt = 0; cnt < fc; cnt++) {
			...
			amt = (int) Math.floor(((double) (cnt + 1)) * incr)
					- (int) Math.floor(((double) cnt) * incr);
			if (tf.isLR()) {
	...
	void setTextBounds(String str, Node node) {
		...
		String fontname = node.getAttribute(FONTNAME_ATTR).getStringValue()
				.toLowerCase();
		if (fontname.startsWith("courier") || fontname.startsWith("monospaced")) {
			cwFont = true;
		} else if (fontname.startsWith("helvetica")
				|| fontname.startsWith("sansserif")) {
		...
		for (int i = 0; i < array.length; i++) {
			...
			value = array[i] - 32;
			fwidth += (cwFont) ? fontwidth[0]
					: ((value >= 0 && value < fontwidth.length) ? fontwidth[value]
							: 0);
		}
		if (fwidth > xwidth) {
			xwidth = fwidth;
		}
		int height = ((Integer) node.getAttributeValue(FONTSIZE_ATTR))
				.intValue();
