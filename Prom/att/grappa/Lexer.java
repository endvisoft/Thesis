...
public class Lexer {
	/**
	 * First character of lookahead. Set to '\n' initially (needed for initial 2
	 * calls to advance())
	 */
	private int next_char = '\n';

	/**
	 * Second character of lookahead. Set to '\n' initially (needed for initial
	 * 2 calls to advance())
	 */
	private int next_char2 = '\n';

	/**
	 * Current line number for use in error messages. Set to -1 to account for
	 * next_char/next_char2 initialization
	 */
	...
	/**
	 * Create an instance of <code>Lexer</code> that reads from
	 * <code>input</code> and sends error messages to <code>error</code>.
	 * 
	 * @param input
	 *            input <code>Reader</code> object
	 * @param error
	 *            error output <code>Writer</code> object
	 * 
	 * @exception IllegalArgumentException
	 *                whenever <code>input</code> is null
	 */
	public Lexer(Reader input, PrintWriter error)
	 ...
	 * 
	 * @exception IOException
	 *                whenever a problem reading from <code>input</code> is
	 *                encountered
	 */
	 ...
	 * print stream passed to Lexer (if any) and a GraphParserException is
	 * thrown.
	 * 
	 * @param message
	 ...
	 * line number and the position in the line. Messages are printed on print
	 * stream passed to Lexer (if any).
	 * 
	 * @param message
	 *            the message to print.
	 */
	private void emit_warn(String message) {
		if (errWriter != null) {
			errWriter
					.println("WARNING: Lexer" + getLocation() + ": " + message);
		}
		warning_count++;
	}
	...
	/**
	 * Swallow up a quote string. Quote strings begin with a double quote and
	 * include all characters up to the first occurrence of another double quote
	 * (there is no way to include a double quote inside a quote string). The
	 * routine returns a Symbol object suitable for return by the scanner.
	 */
	private Symbol do_quote_string() throws IOException {
		...
		synchronized (cmnstrbuf) {
			cmnstrbuf.delete(0, cmnstrbuf.length()); // faster than
			// cmnstrbuf.setLength(0)!
			...
			while (!(next_char == '"')) {
				...
				// if we have run off the end issue a message and break out of
				// loop
				if (next_char == EOF_CHAR) {
	 ...
	 * underscores or dollar signs. This routine returns an Symbol suitable for
	...
	/**
	 * Return one Symbol. This method is the main external interface to the
	 * scanner. It consumes sufficient characters to determine the next input
	...
	public Symbol next_token(int debugLevel) throws IOException {
		if (debugLevel > 0) {
			...
			if (errWriter != null && debugLevel >= 5) {
				errWriter
						.println("DEBUG: Lexer: next_token() => " + result.sym);
			}
			return result;
		} else {
