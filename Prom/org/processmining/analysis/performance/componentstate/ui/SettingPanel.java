...
public class SettingPanel extends JPanel {
	...
	protected void initEventTypeSelection() {
		...
		while (i < statesList.length) {
			...
			int priority = -1;
			if (tempState.equals(ComponentStateModel.STATE_RUNNING)
					|| tempState.equals(ComponentStateModel.STATE_WORKING)
					|| tempState.equals(ComponentStateModel.STATE_DEFINED)) {
				priority = 1;
				tempColor = new Color(50, 200, 50); // green
			} else if (tempState.equals(ComponentStateModel.STATE_ASSIGNED)
					|| tempState.equals(ComponentStateModel.STATE_UNCONFIRMED)) {
				priority = 2;
				tempColor = Color.YELLOW; //
			} else if (tempState.equals(ComponentStateModel.STATE_SUSPENDED)
					|| tempState.equals(ComponentStateModel.STATE_UNDEFINED)) {
				priority = 3;
				tempColor = new Color(200, 100, 50); // orange
			} else if (tempState.equals(ComponentStateModel.STATE_COMPLETED)) {
				priority = 3;
				tempColor = new Color(35, 130, 170); // nice blue-violet
			} else if (tempState.equals(ComponentStateModel.STATE_ABORTED)) {
				priority = 3;
				tempColor = new Color(200, 50, 200); // violet
			} else if (tempState.equals(ComponentStateModel.STATE_MANUALSKIP)) {
				priority = 3;
		...
		while (i < statesList.length) {
			...
			// set default prioritites:
			int priority = -1;
			if (tempState.equals(ComponentStateModel.STATE_WORKING)) {
				priority = 1;
				tempColor = new Color(50, 200, 50); // green
			} else if (tempState.equals(ComponentStateModel.STATE_ASSIGNED)) {
				priority = 2;
		...
		while (i < statesList.length) {
			...
			// set default prioritites:
			int priority = -1;
			if (tempState.equals(ComponentStateModel.STATE_DEFINED)) {
				priority = 1;
				tempColor = new Color(50, 200, 50); // green
			} else if (tempState.equals(ComponentStateModel.STATE_UNCONFIRMED)) {
				priority = 2;
				tempColor = Color.YELLOW;
			} else {
				priority = 3;
	...
	public JPanel initStatePanel() {
		...
		while (i < statesList.length) {
			...
			if (type.equals(ComponentStateAnalysis.ST_ORIG)
					&& tempState.equals(ComponentStateModel.STATE_UNASSIGNED)) {
