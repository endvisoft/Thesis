...
public class EpmlExport implements ExportPlugin {
	...
	public void export(ProvidedObject object, OutputStream output, String name, String id)
			throws IOException {
		Object[] o = object.getObjects();
		...
		for (int i = 0; i < o.length; i++) {
			if (o[i] instanceof ConfigurableEPC) {
				enumerateEPCs(o[i]);
				epclist = writeFlat((ConfigurableEPC) o[i],name,id);
				break;
			}
		}
		for (int i = 0; i < o.length; i++) {
			if (o[i] instanceof EPCHierarchy) {
				...
				epclist = writeHierarchy(hierarchy,name,id);
	...
	public void writeAll(final OutputStream output) throws IOException {
		...
		writeLn("<epml:epml xmlns:epml=\"http://www.epml.de\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://www.epml.de EPML_2.0.xsd\">", output);
	...
	public String writeFlat(ConfigurableEPC epc, String name, String id) {
		Iterator it = EPCs.iterator();
		String epcs = "";
		while (it.hasNext()) {
			epcs = epcs + writeEPC((ConfigurableEPC) it.next(),name,id);
		}
		return  epcs ;
	}

	public String writeHierarchy(EPCHierarchy hierarchy, String name, String id) {
		Iterator it = hierarchy.getRoots().iterator();
		String epcs = "";
		while (it.hasNext()) {
			Object obj = it.next();
			epcs = epcs + writeHierarchyRecursive(obj, hierarchy, name, id);
		}
		return epcs;
	}

	public String writeHierarchyRecursive(Object obj, EPCHierarchy hierarchy, String name, String id) {
		if (obj instanceof ConfigurableEPC) {
			Message.add("at least cepc");
			ConfigurableEPC cepc = (ConfigurableEPC) obj;
			return writeEPC(cepc,name,id);
		}
		if (obj instanceof String) {
			...
			while (children.hasNext()) {
				childstring = childstring
						+ writeHierarchyRecursive(children.next(), hierarchy, name, id);
			}
	...
	public String writeEPC(ConfigurableEPC epc, String name, String id) {
		String elements = "";
